...
mod tests {
    ...
    fn embedded_migrator_contains_consolidated_initial_schema() {
        ...
        // Production migration 0027 repairs legacy workflow-owner mentions.
        ...
        // Fork-specific channel organization and search migrations preserve the
        // already-deployed migration history before upstream additions.
        ...
        assert!(migrations[29]
            .sql
            .as_str()
            .contains("ADD COLUMN IF NOT EXISTS catalog_section"));
        ...
        // Recovery migration 0034 alters populated tables and must preserve
        // the same fail-fast lock behavior as the deletion migration.
        assert_eq!(migrations[33].version, 34);
        let deletion_recovery = migrations[33].sql.as_str();
    ...
    fn deletion_surface_parity_between_migration_0033_and_schema_sql() {
        ...
        // 0033's ALTER TABLE additions are expressed inline by the
        ...
        for column in &migration.communities_added_columns {
            assert!(
                ...
                "schema.sql communities table is missing 0033 column {column}"
            );
        }
