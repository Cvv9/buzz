...
pub async fn handle_req(
    ...
    let admin_discovery_request =
        !filters.is_empty() && filters.iter().all(is_admin_channel_discovery_filter);
    let discovery_channels = if admin_discovery_request {
        ...
        requested
            ...
            .filter(|channel_id| {
                accessible_channels.contains(channel_id)
                    || (admin_discovery_request && discovery_channels.contains(channel_id))
            })
            .collect::<Vec<_>>()
    });
