...
fn hosted_config_filters(candidate_pubkeys: &[String]) -> Vec<serde_json::Value> {
    candidate_pubkeys
        .iter()
        .map(|pubkey| {
            serde_json::json!({
                ...
                "#d": [pubkey, format!("hosted-agent:{pubkey}")],
                "limit": 2,
            })
        })
        .collect()
...
fn apply_hosted_config_overlays(
    agents: &mut [RelayAgentInfo],
    config_events: Vec<nostr::Event>,
    membership_events: &[nostr::Event],
) {
...
async fn list_relay_agents_for_selection(
    ...
    let config_filters = hosted_config_filters(&candidate_pubkeys);
    ...
    for filters in config_filters.chunks(RELAY_FILTER_BATCH_SIZE) {
        config_events.extend(
            query_relay(state, filters)
                .await
                .map_err(|error| format!("relay hosted-agent config query failed: {error}"))?,
        );
    }
    let admin_membership_events = query_relay(
        state,
        &[serde_json::json!({
    ...
    .await
    .map_err(|error| format!("relay administrator-membership query failed: {error}"))?;
    ...
    apply_hosted_config_overlays(&mut agents, config_events, &admin_membership_events);
    Ok(agents)
}
...
mod tests {
    ...
    #[test]
    fn hosted_config_queries_are_bounded_to_candidate_coordinates() {
        let pubkeys = vec!["a".repeat(64), "b".repeat(64)];

        let filters = hosted_config_filters(&pubkeys);
        ...
        for (filter, pubkey) in filters.iter().zip(pubkeys) {
            ...
            assert_eq!(
                filter["#d"],
                serde_json::json!([pubkey, format!("hosted-agent:{pubkey}")])
            );
