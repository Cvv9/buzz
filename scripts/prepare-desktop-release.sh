...
EOF
if existing="$(gh pr list --repo "$release_repo" --head "$branch" --state open --json number --jq '.[0].number')" && [[ -n "$existing" ]]; then
  gh pr edit --repo "$release_repo" "$existing" --title "chore(release): release Buzz Desktop version $version" --body-file "$body"
else
  gh pr create --repo "$release_repo" --base main --head "$branch" \
