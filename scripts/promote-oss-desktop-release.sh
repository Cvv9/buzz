...
RELEASE_DOWNLOAD_BASE="https://github.com/${REPOSITORY}/releases/download"
...
jq -e --arg version "$VERSION" --arg base "${RELEASE_DOWNLOAD_BASE}/${TAG}" --argjson expected "$EXPECTED_PLATFORMS" '
  .version == $version and
...
while IFS= read -r url; do
  asset="${url##*/}"
  [[ "$url" == "${RELEASE_DOWNLOAD_BASE}/$TAG/$asset" ]] || fail "$CANDIDATE contains non-canonical updater URL: $url"
