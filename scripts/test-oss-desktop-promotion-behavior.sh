...
write_manifest() {
  local file="$1" version="$2" signature="${3-signed}" base_version="${4-$2}" repository="${5-$test_repository}"
  jq -n --arg version "$version" --arg signature "$signature" --arg base "https://github.com/${repository}/releases/download/desktop-v${base_version}" '{
...
#          [second-current] [post-write] [upload-fail] [tag-sha] [target-sha] [version] [repository]
run_case() {
  local name="$1" expected="$2" upload="$3" candidate="$4" current="$5" release="$6"
  local second="${7:-}" post="${8:-}" upload_fail="${9:-false}" tag_sha="${10:-abc123}" target_sha="${11:-abc123}" version="${12:-1.2.3}" repository="${13:-$test_repository}"
  local case_dir="$tmp/$name" output status
  mkdir -p "$case_dir"; : > "$case_dir/count"; rm -f "$case_dir/uploaded"
  set +e
  output="$(PATH="$tmp/bin:$PATH" GITHUB_REPOSITORY="$repository" \
...
portable_repository="example/portable-buzz"
write_manifest "$tmp/portable-candidate.json" 1.2.3 signed 1.2.3 "$portable_repository"
write_manifest "$tmp/portable-current.json" 1.2.2 signed 1.2.2 "$portable_repository"
run_case portable-repository pass yes "$tmp/portable-candidate.json" "$tmp/portable-current.json" "$(release_json)" '' '' false abc123 abc123 1.2.3 "$portable_repository"

echo "desktop updater release behavior passed"
